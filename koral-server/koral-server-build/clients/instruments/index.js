...
async function main($container) {
  ...
  const instrumentState = await client.stateManager.create('instrument', {
    id,
    instrument,
    data
  });
  ...
  instrumentState.onUpdate(updates => {
    for (let [key, value] of Object.entries(updates)) {
      switch (key) {
        case 'instrument':
          {
            viewTagName = instrument in instrumentsViews ? instrumentsViews[instrument] : literal`koral-not-found`;
            break;
          }
      }
    }
